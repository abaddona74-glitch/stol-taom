...
if (
  process.env.SKIP_REDIS_CHECK === "1" ||
  String(process.env.SKIP_REDIS_CHECK).toLowerCase() === "true"
) {
...
async function attemptInstallFromRepo() {
  ...
  for (const dir of scanDirs) {
    ...
    if (redisServer) {
      try {
        log(
          "Found redis-server executable:",
          redisServer,
          "-> starting detached",
        );
        const child = child_process.spawn(redisServer, [], {
          detached: true,
          stdio: "ignore",
        });
        child.unref();
  ...
  err(
    "No suitable installer or redis-server found in",
    installDir,
    "or install/Redis",
  );
  return false;
...
(async function main() {
  ...
  if (process.platform === "win32") {
    ...
    try {
      ...
      err(
        "redis still not responding after install:",
